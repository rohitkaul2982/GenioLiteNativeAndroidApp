...
import {Platform,
 ...
 Linking,
 ProgressBarAndroid,
  ActivityIndicator, AsyncStorage, NetInfo, AppRegistry, Alert} from 'react-native';
...
export default class GenioWebView extends Component {
  ...
  onNavigationStateChange(navState){
    ...
    var pattern1 = new RegExp("processTransaction");
    var is_transaction = pattern1.test(url);

    var pattern2 = new RegExp("genio_lite");
    var is_geniolite = pattern2.test(url);

    var pattern3 = new RegExp("genio_cpm");
    var is_cpm = pattern3.test(url);

    var pattern4 = new RegExp("tel:");
    var is_tel = pattern4.test(url);
    const open_outside = url.includes('OPEN_OUTSIDE_APP=1') || url.includes('.pdf');
   // Alert.alert(open_outside);
    if ((is_transaction || is_cpm || is_tel || open_outside)){
      Linking.canOpenURL(url).then(supported => {
        ...
        console.error('An error occurred',url, err)
      });
    }
  }
  ...
  msgHandler(event,thisObj){
      ...
      if(self.IsJsonString(event.nativeEvent.data)){
          ...
          if(msg.type == 'download'){
            ...
            self.props.screenProps.recordCalls(msg.tel,msg.msg,msg.compname,msg.mobileNum);
            return false;
          } else if(msg.showRecord) {
            self.props.navigation.push('RecordingView', { empcode: self.state.empcode});
  ...
  render() {
    ...
    const uri_source =this.state.url+"redirect_url=5&empcode="+ this.state.empcode + "&auth_token=TUFEaRsasqqNlUYcdeKmnaaKrrxWbdgR&SERVICE_PARAM=127";
    var thisObj = this;
    return (
      <View style= {{'flex': 1,paddingTop:this.state.padding}}>
        ...
        <CustomWebView
          ...
          onMessage={(e) => this.msgHandler(e, thisObj)}
