...
export function registerAppListener(navigation,webview){
  //console.log('AppState.currentState Listener--',AppState.currentState,navigation,webview);
  FCM.on(FCMEvent.Notification, notif => {
    ...
    if(notif.opened_from_tray){
      //webview ? webview.postMessage(JSON.stringify(genio_route)) :  navigation.push('Login',{'notification_url':1});
      webview && webview.postMessage(JSON.stringify(genio_route));

      console.log('AppState.currentState Listener opened_from_tray',AppState.currentState);  
      if(AppState.currentState == 'background'){
        console.log('L to L')
        navigation.push('Login',{'notification_url':1})
      }    
...
export function registerAppListener_background(){
  //console.log('AppState.currentState Listener--',AppState.currentState,navigation,webview);
  console.log('registerAppListener_background --> ')
  FCM.on(FCMEvent.Notification, notif => {
